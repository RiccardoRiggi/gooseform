...
export default function GooseColorField(inp: any) {
    ...
    let config: GooseColorFieldType = inp.input;
    ...
    return (<>
        <input type={"color"} onChange={aggiornaStato} className='form-control' id={id} name={config.name} disabled={config.disabled} readOnly={config.readonly} value={formData[id]} />
    </>);
